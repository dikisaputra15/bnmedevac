    ...
    data.forEach(airport => {
        ...
        const popupContent = `
            ...
            <strong>Address:</strong>
                ...
                ${airport.province_name ? ', ' + airport.province_name : ''}, Brunei Darussalam <br>
...
async function applyAirportFilters() {
    ...
    // Tetap toleran kalau suatu saat endpoint mengirim array polos.
    const airports = Array.isArray(result) ? result : (result.airports || []);
    const categoryCounts = (!Array.isArray(result) && result.categoryCounts) ? result.categoryCounts : null;
    ...
    if (totalEl) {
        totalEl.innerHTML = `<strong>Airports:</strong> ${filteredAirports.length}`;
    }
    ...
    const counts = categoryCounts || (() => {
        ...
        airports.forEach(a => {
            ...
            a.category.split(',').forEach(cat => {
                cat = cat.trim();
                if (!cat) return;
                local[cat] = (local[cat] || 0) + 1;
            });
        });
        return local;
    })();

    // Ambil kategori langsung dari checkbox yang ter-render, supaya daftarnya
    // tidak pernah beda dengan yang ada di panel filter.
    document.querySelectorAll('input[name="airportClass"]').forEach(cb => {
        const badge = document.getElementById(`count-${cb.value}`);
