...
drawButton.addEventListener('click', () => {
    ...
    if (isDrawingPolygon) {
        ...
        activePolyline = new google.maps.Polyline({
            path: polygonLatLngs, strokeColor: '#0000FF', strokeOpacity: 0.8, strokeWeight: 3, clickable: false, map
        });
        cursorPolyline = new google.maps.Polyline({
            path: [], strokeColor: '#0000FF', strokeOpacity: 0.5, strokeWeight: 3, clickable: false, map
        });
        startMarker = null;
...
async function finishPolygon() {
    ...
    if (polygonLatLngs.length > 2) {
        ...
        activePolygon = new google.maps.Polygon({
            paths: polygonLatLngs, strokeColor: '#0000FF', strokeOpacity: 0.8, strokeWeight: 3,
            fillColor: '#0000FF', fillOpacity: 0.2, editable: true, map
        });
        ...
        if (polygonLatLngs.length === 1) {
            startMarker = new google.maps.Marker({
                ...
                icon: {
                    ...
                    fillColor: '#FFFFFF', fillOpacity: 1, strokeColor: '#0000FF', strokeWeight: 2
                },
                zIndex: 999
            });
...
async function applyEmbassyFilters() {
    ...
    const result = await fetchEmbassyData(filters);

    // /api/embassy mengembalikan array polos.
    // Tetap toleran kalau suatu saat endpoint mengirim { embassies: [...] }.
    const embassies = Array.isArray(result) ? result : (result.embassies || []);

    addEmbassyMarkers(embassies);

    const totalEl = document.getElementById('totalCountDisplay');
    if (totalEl) {
        totalEl.innerHTML = `<strong>Embassies:</strong> ${embassies.length}`;
    }
}
...
document.addEventListener('click', async e => {
    ...
    if (e.target.id === 'resetMapFilter') {
        ...
        if (typeof $ !== 'undefined' && $.fn && $.fn.select2) {
            $('#embassy_name_map').val(null).trigger('change');
        } else {
            const n = document.getElementById('embassy_name_map');
            if (n) n.value = '';
        }
